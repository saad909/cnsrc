...
class inventory_mgmt_func(QDialog):
    ...
    def create_dictionary( self,hostname , ip_address, username,password, secret ,device_type_index ):
        device_type = None
    ...
    def check_hostname_duplication(self,all_hosts,host): 
        for device in all_hosts:
            if device["hostname"] == host['hostname']:
                ...
                QMessageBox.information(self,"Warning","Host name duplication occured")
    ...
    #2.return True in case of duplication -> True,False
    #3. True -> duplication occurs
    def check_ip_duplication(self,all_hosts,host): # return True in case of duplication
        for device in all_hosts:
            if device['data']["host"] == host['data']['host']:
                print(f"{device['hostname']} has the same ip address ")
                QMessageBox.information(self,"Warning",f"{device['hostname']} has the same ip address ")
    ...
    def add_host_into_inventory(self,device):
        ...
        if host_file_not_empty:
            ...
            hostname_duplication = self.check_hostname_duplication(all_devices_list,device)
            ...
            print('-----------------------ALL Devices in host file --------------------------------')
