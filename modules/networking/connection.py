...
class connection(QDialog):
    ...
    def create_handler(self, device):
        # decrypt deivce passwords
        device["data"]["password"] = self.decrypt_password(device["data"]["password"])
        ...
        try:
            ...
            QMessageBox.information(
                self, "Note", f"For {hostname} authentication Failed"
            )
            return False
            ...
            QMessageBox.information(
                self, "Note", f"SSH error. Make sure ssh is enabled on {hostname}"
            )
            print(str(error))
